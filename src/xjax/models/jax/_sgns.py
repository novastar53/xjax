...
def _batch(
    *, rng: Array, vocab_size: int, batch_size: int, neg_per_pos: int, dataset: Sequence
) -> Tuple[Sequence]:
    ...
    neg_idxs = jax.random.randint(
        k2, (neg_per_pos * batch_size, 2), minval=0, maxval=vocab_size - 1
    )
...
def _loss(model, K, params, X_batch) -> float:
    ...
    loss = optax.losses.sigmoid_binary_cross_entropy(
        pos_logits, jnp.ones(pos_logits.size)
    ).mean()
    ...
    loss += (
        ...
        * optax.losses.sigmoid_binary_cross_entropy(
            neg_logits, jnp.zeros(neg_logits.size)
        ).mean()
    )
